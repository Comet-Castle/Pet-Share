...
This document records package, platform, and tooling choices for Pet Share. Use it to keep the project low-cost, license-aware, and focused.

Do not treat an entry here as permission to install a dependency automatically. Add packages only when current scoped work needs them, then update this log and `ATTRIBUTIONS.md` when attribution is required.
...
- `Accepted`: planned for implementation when relevant scoped work starts.
...
| Sanity TypeGen | Accepted | Development tooling | Free | Open source / Sanity tooling terms | Generated query/document types | Reduces hand-written duplicate Sanity types and keeps GROQ result types aligned. | Run `pnpm typegen` when schemas or queries change. | No user-facing attribution expected. |
| Sanity Visual Editing / Presentation Tool | Accepted | Runtime/Studio capability | Verify current Sanity plan requirements before launch | Sanity package and service terms | Presentation preview, Draft Mode, and visual editing overlays | Implemented as the editorial preview workflow for the CMS-backed demo. | Keep preview tokens server-only except the optional least-privilege browser read token for draft-capable live updates. | Covered under Sanity package/service attribution. |
...
| Framer Motion / Motion | Deferred | Runtime animation dependency | Free package; verify current package and license before install | Open source; verify current package terms | Scroll-triggered animation, staggered choreography, and more complex UI motion if needed | Preferred escalation path if Tailwind/CSS animation becomes awkward or if scroll-triggered patterns become common. | Do not add day one. Avoid building a broad custom Intersection Observer animation framework before considering this. The current scroll-entry need is met by one small shared `useInView` hook plus a `Reveal` CSS wrapper (`lib/hooks/use-in-view.ts`, `components/ui/reveal.tsx`), not a broad framework, so no dependency was added. Revisit if staggered choreography or complex motion outgrows the CSS approach. | Required if installed. |
...
| Nunito Sans | Accepted | Font asset | Free | SIL Open Font License 1.1 | Body, navigation, buttons, forms, utility UI | Rounded, friendly, readable baseline font. | Other rounded sans fonts are possible if readability suffers. | Added to `ATTRIBUTIONS.md`. |
| Quicksand | Accepted | Font asset | Free | SIL Open Font License 1.1 | Headings and brand-forward display moments | Friendly rounded display feel without feeling childish. | Could be replaced after visual testing. | Added to `ATTRIBUTIONS.md`. |
...
| Custom SVG pet icons | Accepted | Project asset | Project-owned | Project-owned unless adapted from another source | Pet type icons that Lucide does not cover | Keeps pet filters visually complete while matching Lucide style. | Current custom dog/cat/rabbit SVG icons are project-owned seed media assets. Avoid copying third-party SVGs without license review. | No third-party attribution if original. |
| Vitest | Accepted | Development/testing dependency | Free | MIT | Unit and lightweight integration tests | Lightweight test runner for utilities, seed validation, route handlers, and logic. Installed during initial app scaffolding with a minimal utility test. | Jest is heavier; Playwright is for browser checks later. | Added to `ATTRIBUTIONS.md`. |
...
| Playwright | Accepted | Development/testing dependency | Free | Apache-2.0 | Browser smoke tests, responsive checks, accessibility-oriented interaction checks | Available for targeted browser verification when manual checks are not enough. | Do not expand into broad E2E coverage unless a future feature justifies it. | Added to `ATTRIBUTIONS.md`. |
| ESLint | Accepted | Development tooling | Free | MIT | Linting TypeScript, React, Next.js, Sanity, and scripts | Baseline code-quality gate. Installed during initial app scaffolding using Next's flat `core-web-vitals` config. | Use Next.js defaults where possible. | Added to `ATTRIBUTIONS.md`. |
...
## Initial Scaffold Installed Package Notes
