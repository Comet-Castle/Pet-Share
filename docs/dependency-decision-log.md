...
| Tailwind animation utilities | Likely | Styling helper | Free if using open-source package or custom CSS | Verify package license before install | Entry animations and subtle hover motion | The design calls for gentle motion, but avoid adding a larger animation library on day one. | Prefer Tailwind CSS utilities and small custom CSS first; consider `tailwindcss-animate` only when useful. | Record if a package is added. |
...
| Vitest | Accepted | Development/testing dependency | Free | Open source | Unit and lightweight integration tests | Lightweight test runner for utilities, seed validation, route handlers, and logic. | Jest is heavier; Playwright is for browser checks later. | Record if required by final license review. |
...
| ESLint | Accepted | Development tooling | Free | Open source | Linting TypeScript, React, Next.js, Sanity, and scripts | Baseline code-quality gate. | Use Next.js defaults where possible. | No user-facing attribution expected. |
...
| Mailgun | Accepted | External service | Use existing/free account; verify current sending limits | Mailgun service terms | Phase-one form email delivery to the master inbox | User has a free Mailgun account; supports real email sending without building a mail server. | SendGrid was rejected because its free/trial constraints were not ideal. | No public attribution expected. |
...
- Confirm exact Sanity package set during scaffolding.
- Confirm whether Tailwind animation can be handled with custom CSS or needs an add-on package.
- Confirm whether Testing Library is needed in Milestone 2 or can wait for interactive components.
